...
function saveTasks() {
  saveAndSync(BHB.TASK_KEY, tasks);
}
  ...
  list.forEach(function (t) {
    ...
    $list.append(
      '<div class="task-item ' +
        ...
        " " +
        dueLabel(t.due) +
        (t.hours != null && t.hours !== ""
          ? ' <span class="badge bg-primary-subtle text-primary"><i class="bi bi-clock me-1"></i>' + t.hours + " hrs</span>"
          : "") +
        "</div>" +
        "</div>" +
        '<div class="dropdown">' +
...
function openEditTask(id) {
  ...
  $("#taskDue").val(t.due || "");
  $("#taskHours").val(t.hours != null && t.hours !== "" ? t.hours : "");
  fillAssigneeDatalist();
  taskModal.show();
}
...
function saveTask() {
  ...
  var id = $("#taskId").val();
  var hoursRaw = ($("#taskHours").val() || "").trim();
  var hoursVal = hoursRaw === "" ? null : parseFloat(hoursRaw);
  if (hoursRaw !== "" && (isNaN(hoursVal) || hoursVal < 0)) {
    showToast("Hours must be a valid number (0 or more).", "warning");
    $("#taskHours").focus();
    return;
  }
...
function csvCell(val) {
  ...
  // Only quote if needed (comma, quote, or leading/trailing space)
  if (/[",\n]/.test(s) || s !== s.trim()) {
    return '"' + s.replace(/"/g, '""') + '"';
  }
  return s;
...
function exportTasksCSV() {
  ...
  var header = ["Company", "Title", "Assignee", "Priority", "Status", "Due Date", "Hours Gathered", "Description", "Created"];
  ...
  a.href = url;
  a.download = "BHB_Tasks_" + todayISO() + ".csv";
  a.click();
  URL.revokeObjectURL(url);
  showToast("Tasks exported (" + list.length + " rows). Open in Excel / Google Sheets.", "success");
}
...
$(function () {
  ...
  function startApp() {
    ...
    $("#btnExportTasks").on("click", exportTasksCSV);
